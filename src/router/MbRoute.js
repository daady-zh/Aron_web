...
const loadView = (module, view) => {
  return () => import(`@/views/${module}/Mb${view}.vue`)
}
// App端路由
const routes = [
  // 一级路由
  ...
  },
  {
    ...
    name: 'login',
    meta: { title: '登录页' },
    component: () => import('@/views/login/MbLogin.vue')
  },
  {
    path: '/pdfview',
    name: 'pdfview',
  ...
  // 架子
  {
    path: '/layout',
    meta: { title: '首页' },
    component: () => import('@/views/layout/MbLayout.vue'),
    redirect: '/home',
    // 二级路由
    children: [
      {
      ...
      },
      {
        ...
        name: 'modules',
        meta: { title: '插件分类' },
        component: loadView('layout', 'Modules')
      },
      {
        path: '/apartment',
        name: 'apartment',
        meta: { title: '部门模块' },
        component: loadView('layout', 'Apartment')
      },
      {
        path: '/user',
        name: 'user',
        meta: { title: '用户信息' },
        component: () => import('@/views/user/MbUser.vue')
      }
    ]
  }
]
